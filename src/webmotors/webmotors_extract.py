...
def __load_car(car) -> dict:
    ...
    tmp_row = pd.concat([tmp_row, __load_specs(car['Specification'])], axis=1)
    tmp_row = pd.concat([tmp_row, __load_seller(car['Seller'])], axis=1)
    tmp_row = pd.concat([tmp_row, __load_prices(car['Prices'])], axis=1)
    ...
    tmp_row = pd.concat([tmp_row, __get_optionals(__make_opt_url(tmp_row))], axis=1)
